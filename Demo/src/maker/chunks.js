    ...
    .map((ch, i) => {
      ...
      return `<div class="chunk-card" draggable="true" data-idx="${esc(i)}" data-id="${esc(ch.id)}">
        <div class="chunk-header">
          ...
          <span class="chunk-type-badge" data-type="${esc(ch.type)}">${esc(ch.type)}</span>
          <span class="chunk-title-display">${esc(ch.title)}</span>
          <div class="chunk-header-right">
            <span class="chunk-comp-badge">${ch.compression ? "LZ4" : "Raw"}</span>
            <button class="chunk-settings-btn icon-btn" data-action="settings" data-id="${esc(ch.id)}" title="Chunk settings">${icon("Settings", 14)}</button>
            <button class="chunk-delete-btn icon-btn" data-action="delete" data-id="${esc(ch.id)}" title="Remove chunk">${icon("Trash2", 14)}</button>
          </div>
        </div>
        ${renderChunkBody(ch, i)}
        <div class="chunk-settings-panel" id="settings_${esc(ch.id)}" style="display:none">
          <div class="chunk-settings-inner">
            <label>Compression: <select class="chunk-sel-comp" data-id="${esc(ch.id)}">
            ...
            ${
              ch.type === "COVR" || ch.type === "IMG_"
                ? `
            <label>Cover Type: <select class="chunk-sel-cover" data-id="${esc(ch.id)}">
            ...
            ${
              ch.type === "IMG_" || ch.type === "COVR"
                ? `
            <label>Alt Text: <input type="text" class="chunk-input-alt" value="${esc(ch.altText || "")}" placeholder="Image description" data-id="${esc(ch.id)}" /></label>`
                : ""
            }
            ${
              ch.type === "FONT"
                ? `
            <label>Embedding: <select class="chunk-sel-embed" data-id="${esc(ch.id)}">
            ...
            <label>License URL: <input type="text" class="chunk-input-license" value="${esc(ch.fontLicenseUrl || "")}" placeholder="https://..." data-id="${esc(ch.id)}" /></label>`
                : ""
            }
          </div>
        </div>
...
function renderChunkBody(ch, i) {
  ...
  if (t.markup) {
    return `<div class="chunk-body">
      <div class="chunk-title-row">
        <input type="text" class="chunk-title-input" value="${esc(ch.title)}" placeholder="Title" data-id="${esc(ch.id)}" />
        <select class="chunk-type-select" data-id="${esc(ch.id)}">
  ...
  if (t.binary) {
    ...
    return `<div class="chunk-body">
      <div class="chunk-title-row">
        <input type="text" class="chunk-title-input" value="${esc(ch.title)}" placeholder="${isCover ? "Cover title" : isFont ? "Font name" : "Image title"}" data-id="${esc(ch.id)}" />
      </div>
      <div class="chunk-file-area">
        ...
        <div class="chunk-file-preview">
          ...
          <button class="btn btn-secondary" data-action="remove-file" data-id="${esc(ch.id)}" style="font-size:0.8rem;padding:4px 10px;height:auto">Remove</button>
        </div>`
            : `
        <div class="chunk-file-drop" data-id="${esc(ch.id)}">
          <span>Drop ${isCover ? "cover image" : isFont ? "font" : "image"} or click to browse</span>
          <input type="file" class="chunk-file-input" accept="${accept}" data-id="${esc(ch.id)}" />
        </div>`
        }
      </div>
    </div>`;
  ...
  if (t.text) {
    return `<div class="chunk-body">
      <div class="chunk-title-row">
        <input type="text" class="chunk-title-input" value="${esc(ch.title)}" placeholder="Stylesheet name" data-id="${esc(ch.id)}" />
      </div>
      <textarea class="chunk-content chunk-css-content" data-id="${esc(ch.id)}" placeholder="/* CSS */" rows="8" spellcheck="false">${esc(ch.content)}</textarea>
    </div>`;
  }
  if (t.math) {
    return `<div class="chunk-body">
      <div class="chunk-title-row">
        <input type="text" class="chunk-title-input" value="${esc(ch.title)}" placeholder="Math title" data-id="${esc(ch.id)}" />
        <select class="chunk-math-select" data-id="${esc(ch.id)}">
          <option value="0" ${ch.mathType === 0 ? "selected" : ""}>MathML</option>
          <option value="1" ${ch.mathType === 1 ? "selected" : ""}>LaTeX</option>
        </select>
      </div>
      <textarea class="chunk-content chunk-math-content" data-id="${esc(ch.id)}" placeholder="${ch.mathType ? "\\sum_{n=1}^{\\infty} ..." : "<math>...</math>"}" rows="8" spellcheck="false">${esc(ch.content)}</textarea>
    </div>`;
  }
  return `<div class="chunk-body"><div class="chunk-title-row">
    <input type="text" class="chunk-title-input" value="${esc(ch.title)}" placeholder="Name" data-id="${esc(ch.id)}" />
  </div></div>`;
}
