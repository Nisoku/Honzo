...
function serializeState() {
  ...
  return {
    ...
    chunks: chunks.map((ch) => ({
      ...ch,
      fileData:
        ch.fileData instanceof Uint8Array
          ? uint8ArrayToBase64(ch.fileData)
          : null,
    })),
...
export function deserializeState(state) {
  ...
  setChunks(
    (state.chunks || []).map((ch) => ({
      ...ch,
      fileData:
        ch.fileData && typeof ch.fileData === "string"
          ? base64ToUint8Array(ch.fileData)
          : null,
    })),
  );
...
export function restoreAutosave() {
  try {
    ...
    if (!raw) {
      showStatus("error", "No auto-save found");
      return;
    }
...
export function renderSaves() {
  ...
  if (names.length === 0 && !autoHtml) {
    list.innerHTML =
      '<div class="maker-saves-empty">No saved projects yet.</div>';
    return;
  }
  list.innerHTML =
    (autoHtml ? autoHtml : "") +
